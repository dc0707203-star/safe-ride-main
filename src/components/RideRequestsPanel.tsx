...
import { MapPin, Phone, User, Clock, CheckCircle, X, AlertCircle, Navigation, RefreshCw } from 'lucide-react';
...
/**
 * Opens maps navigation to the pickup location
 */
const openMapsNavigation = (lat: number | null, lng: number | null, locationName: string) => {
  if (!lat || !lng) {
    // Fallback: open Google Maps search for the location name
...
export const RideRequestsPanel: React.FC<RideRequestsPanelProps> = ({ driverId }) => {
  ...
  if (!driverId) {
    return (
      <div className="p-6 rounded-lg border-2 border-yellow-300 bg-yellow-50">
        <div className="flex items-start gap-3">
          <AlertCircle className="h-5 w-5 text-yellow-600 flex-shrink-0 mt-0.5" />
          <div>
            <h3 className="font-bold text-yellow-800">Driver Profile Loading</h3>
            <p className="text-sm text-yellow-700 mt-1">Waiting for driver data...</p>
          </div>
        </div>
      </div>
    );
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-red-600 mt-2">
              This may be due to RLS permissions or database issues. Please refresh the page or contact support.
            </p>
          </div>
        </div>
      </div>
      ...
      {pendingRequests.length === 0 && acceptedRequests.length === 0 && (
        <div className="text-center py-12">
          ...
          <div className="mt-4 pt-4 border-t border-gray-200">
            <p className="text-2xs text-gray-400">Driver ID: {driverId}</p>
            <p className="text-2xs text-gray-400">Pending: {pendingRequests.length} | Accepted: {acceptedRequests.length}</p>
          </div>
        </div>
      )}
    </div>
      ...
      <CardContent className="p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <div className="flex flex-col sm:flex-col gap-2 sm:justify-between sm:min-w-max">
            ...
            <Button
              size="sm"
              onClick={() => openMapsNavigation(request.pickup_lat, request.pickup_lng, request.pickup_location)}
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <div className="flex flex-col gap-2 sm:justify-center">
            ...
            <Button
              size="sm"
              onClick={() => openMapsNavigation(request.pickup_lat, request.pickup_lng, request.pickup_location)}
              className="bg-indigo-600 hover:bg-indigo-700 text-white text-sm gap-2 flex-shrink-0"
            >
