...
export const useRescueData = () => {
  ...
  const fetchAlerts = async () => {
    try {
      ...
      // Query alerts with student relationship
      ...
      console.log("[useRescueData] Query result:", { data, error });
      ...
      const formattedAlerts: Alert[] =
        data?.map((alert: any) => {
          // Priority: stored name > relationship name > fallback
          let studentName = alert.student_full_name;
          
          if (!studentName && alert.students) {
          ...
          console.log("[useRescueData] Alert formatting:", {
            alertId: alert.id,
            storedName: alert.student_full_name,
            relationshipData: alert.students,
