    ...
    const checkAuth = async () => {
      try {
        ...
        try {
          ...
          console.log("No active trip or error fetching trip:", tripError);
        }
      } catch (error) {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 px-4 py-6 max-w-md mx-auto">
        ...
        {activeTrip && (
          <div className="mb-8 relative group overflow-hidden">
            ...
            <div className="absolute -inset-0.5 bg-gradient-to-r from-emerald-500 via-green-400 to-emerald-500 rounded-2xl blur opacity-75 group-hover:opacity-100 transition duration-300 animate-pulse" />
            
            <div className="relative bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 border-2 border-emerald-400/60 rounded-2xl p-5 shadow-2xl shadow-emerald-500/20 backdrop-blur-sm">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div className="flex-1">
                  <p className="text-emerald-300 text-xs font-black uppercase tracking-widest">Active Trip</p>
                  <p className="text-white font-black text-base">Ride in Progress</p>
                </div>
              </div>

              {/* Student Info Card */}
              <div className="bg-gradient-to-br from-emerald-500/20 to-green-500/20 border-2 border-emerald-400/40 rounded-xl p-4 mb-4 shadow-lg">
                <div className="flex items-center gap-3">
                  {activeTrip.student?.photo_url ? (
                    <img
                      src={activeTrip.student.photo_url}
                      alt={activeTrip.student.full_name}
                      className="h-14 w-14 rounded-full object-cover border-3 border-emerald-400/70 shadow-lg"
                    />
                  ) : (
                    <div className="h-14 w-14 rounded-full bg-emerald-500/40 flex items-center justify-center border-3 border-emerald-400/70 shadow-lg">
                      <User className="h-7 w-7 text-emerald-300" />
                    </div>
                  )}
                  <div className="flex-1 min-w-0">
                    <p className="text-white font-bold text-base truncate">{activeTrip.student?.full_name}</p>
                    {activeTrip.student?.contact_number && (
              ...
              <div className="space-y-2.5 mb-4">
                {activeTrip.start_location_lat && (
                  <div className="flex gap-3 bg-white/5 border border-emerald-400/30 rounded-lg p-3 group/location hover:bg-white/10 transition">
                    <MapPin className="h-5 w-5 text-emerald-400 flex-shrink-0 mt-0.5" />
                    <div className="flex-1">
                      <p className="text-emerald-200 font-semibold text-xs uppercase tracking-wide">Pickup Location</p>
                      <p className="text-emerald-100 text-xs mt-0.5 font-mono">{activeTrip.start_location_lat.toFixed(4)}, {activeTrip.start_location_lng?.toFixed(4)}</p>
                    </div>
                  </div>
                )}
                {activeTrip.end_location_lat && (
                  <div className="flex gap-3 bg-white/5 border border-emerald-400/30 rounded-lg p-3 hover:bg-white/10 transition">
                    <MapPin className="h-5 w-5 text-emerald-300 flex-shrink-0 mt-0.5" />
                    <div className="flex-1">
                      <p className="text-emerald-200 font-semibold text-xs uppercase tracking-wide">Destination</p>
                      <p className="text-emerald-100 text-xs mt-0.5 font-mono">{activeTrip.end_location_lat.toFixed(4)}, {activeTrip.end_location_lng?.toFixed(4)}</p>
                    </div>
                  </div>
                )}
              </div>
              ...
              {/* Time Started */}
              <div className="bg-gradient-to-r from-emerald-500/10 to-green-500/10 border border-emerald-400/40 rounded-lg px-3 py-2.5 flex items-center gap-2.5 text-xs text-emerald-200 font-semibold">
      ...
      <Dialog open={showPolicyModal} onOpenChange={setShowPolicyModal}>
        <DialogContent className="bg-gradient-to-br from-amber-50 via-white to-emerald-50 border-none text-slate-900 max-w-2xl rounded-3xl shadow-2xl overflow-hidden p-0 max-h-[90vh] flex flex-col">
          {/* Header */}
      ...
      <Dialog open={showQRModal} onOpenChange={setShowQRModal}>
        <DialogContent className="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 border-[#CCFF00]/20 text-white max-w-sm rounded-3xl shadow-2xl shadow-black/80">
          <DialogHeader>
