...
import { 
  ArrowLeft, LogOut, User, Mail, Phone, BookOpen, Bell, Lock, HelpCircle, 
  FileText, Shield, Smartphone, ChevronRight, Download, AlertCircle, Key, Zap
} from "lucide-react";
...
const StudentSettings = () => {
  ...
  useEffect(() => {
    const fetchStudentData = async () => {
      try {
        ...
        // Load widget enabled state from localStorage
        const savedWidgetState = localStorage.getItem("widget_sos_enabled");
        setWidgetEnabled(savedWidgetState === "true");
      } catch (error) {
  ...
  const handleWidgetToggle = () => {
    ...
    localStorage.setItem("widget_sos_enabled", newState.toString());
    
    if (newState) {
      toast.success("Widget enabled!", {
        description: "Long-press your home screen and add the SafeRide widget for quick emergency access.",
        duration: 5000,
      });
    } else {
      toast.info("Widget disabled", {
        description: "The widget has been disabled but won't be removed from your home screen.",
      });
    }
  };
      ...
      <div className="px-4 py-6 pb-24 max-w-2xl">
        ...
        <div className="mb-8 p-5 rounded-2xl border border-gray-200 bg-gradient-to-br from-gray-50 to-white">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-black font-bold text-base">{student?.full_name}</p>
              <p className="text-gray-600 text-xs mt-0.5">{student?.student_id_number}</p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-0 mb-6 rounded-2xl border border-gray-200 overflow-hidden bg-white">
          <div className="flex items-start gap-4 p-4 border-b border-gray-100">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-black font-semibold text-sm mt-1 break-all">{student?.email}</p>
            </div>
          </div>
          ...
          <div className="flex items-start gap-4 p-4 border-b border-gray-100">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-black font-semibold text-sm mt-1">{student?.contact_number}</p>
            </div>
          </div>

          {student?.course && (
            <div className="flex items-start gap-4 p-4">
              <BookOpen className="h-5 w-5 text-gray-700 mt-0.5 flex-shrink-0" />
              <div className="flex-1 min-w-0">
                <p className="text-gray-600 text-xs font-semibold uppercase">Course</p>
                <p className="text-black font-semibold text-sm mt-1">{student.course}</p>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="space-y-0 mb-6 rounded-2xl border border-gray-200 overflow-hidden bg-white">
          ...
          >
            <div className="flex items-center gap-3 flex-1">
              ...
              <div className="text-left">
                <p className="text-black font-semibold text-sm">Home Screen Widget</p>
                <p className="text-gray-600 text-xs mt-0.5">Quick emergency SOS access</p>
              </div>
            </div>
            <div className={`w-12 h-7 rounded-full transition-all flex items-center px-1 ${widgetEnabled ? 'bg-green-600' : 'bg-gray-300'}`}>
              <div className={`w-5 h-5 bg-white rounded-full transition-transform ${widgetEnabled ? 'translate-x-5' : ''}`} />
            </div>
          </button>
          <div className="border-b border-gray-100" />
          <SettingItem 
