            ...
            {activeAlerts.length > 0 ? (
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                ...
                <div className="lg:col-span-2">
                  <Card className="h-[700px] rounded-xl overflow-hidden bg-black/40 border-white/20" style={{ overflow: 'visible' }}>
                    {selectedAlertForMap && selectedAlertForMap.location_lat && selectedAlertForMap.location_lng ? (
                      <EmergencyAlertMap
                        alertId={selectedAlertForMap.id}
                        initialAlert={{
                          student_name: selectedAlertForMap.students?.full_name || selectedAlertForMap.student_full_name,
                          photo_url: selectedAlertForMap.students?.photo_url,
                          contact_number: selectedAlertForMap.students?.contact_number,
                          student_id_number: selectedAlertForMap.students?.student_id_number || selectedAlertForMap.student_id_number,
                          level: selectedAlertForMap.level,
                        }}
                      />
                    ) : activeAlerts[0]?.location_lat ? (
                      <EmergencyAlertMap
                        alertId={activeAlerts[0].id}
                        initialAlert={{
                          student_name: activeAlerts[0].students?.full_name || activeAlerts[0].student_full_name,
                          photo_url: activeAlerts[0].students?.photo_url,
                          contact_number: activeAlerts[0].students?.contact_number,
                          student_id_number: activeAlerts[0].students?.student_id_number || activeAlerts[0].student_id_number,
                          level: activeAlerts[0].level,
                        }}
                      />
                    ) : (
                      <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-black to-emerald-950">
                        <div className="text-center">
                          <MapPin className="h-16 w-16 text-emerald-400/30 mx-auto mb-3" />
                          <p className="text-emerald-200/70">No location data available</p>
                        </div>
                      </div>
                    )}
                  </Card>
                ...
                <div className="lg:col-span-1">
                  <Card className="rounded-xl bg-black/40 border-white/20 overflow-hidden flex flex-col h-[700px]">
                    <div className="bg-black/60 border-b border-white/10 px-4 py-3">
                      <p className="text-sm font-bold text-red-300">🚨 {activeAlerts.length} Active Alert{activeAlerts.length !== 1 ? 's' : ''}</p>
                    </div>
                          ...
                          >
                            <div className="flex gap-3 items-start">
                              {alert.students?.photo_url ? (
                                ...
                                <div className="w-10 h-10 rounded-lg bg-gray-600/30 flex items-center justify-center flex-shrink-0 text-lg">
                                  👤
                                </div>
                              )}
                              <div className="flex-1 min-w-0">
                                <p className="text-sm font-bold text-white truncate">
                                  {alert.students?.full_name || alert.student_full_name || 'Unknown'}
                                </p>
                                <p className="text-xs text-gray-300">{alert.level}</p>
                ...
                {/* Full width alerts grid below map */}
                        ...
                        <div className="flex flex-col lg:flex-row lg:items-start gap-5 sm:gap-6">
                              ...
                              <div className="flex items-start justify-between gap-2 mb-2 flex-wrap">
                                <div className="flex-1">
                                  <h3 className="font-bold text-lg sm:text-2xl text-gray-900 truncate group-hover:text-gray-700 transition-colors">
                                    {alert.student_full_name || alert.students?.full_name || "Unknown"}
                                  </h3>
                                  <p className="text-xs sm:text-sm text-gray-600 font-mono mt-0.5">
                                    📌 {alert.student_id_number || alert.students?.student_id_number || "N/A"}
                                  </p>
                                </div>
                          ...
                          <div className="flex flex-col gap-2.5 w-full lg:w-auto lg:flex-shrink-0 sm:flex-row lg:flex-col">
                            {alert.students?.contact_number && (
                              <Button
                                size="sm"
                                className="gap-2 bg-gradient-to-r from-emerald-600 to-emerald-700 hover:from-emerald-700 hover:to-emerald-800 text-white font-semibold text-xs sm:text-sm shadow-lg hover:shadow-xl transition-all active:scale-95 rounded-xl py-2 px-4"
                                onClick={(e) => {
                                  e.stopPropagation();
              ...
              <div className="text-center py-16">
                ...
                <h3 className="text-xl font-semibold text-emerald-50 mb-2">All Clear</h3>
                <p className="text-emerald-100/80">No active emergencies at this time</p>
              </div>
            )}
          </div>
        )}
          ...
          <div>
            {resolvedAlerts.length > 0 ? (
              ...
              <div className="text-center py-12">
                ...
                <p className="text-emerald-100/80">No resolved alerts yet</p>
              </div>
            )}
          </div>
        )}
